...
public class ImportPrivateKeysSubmitActionTest extends TestCase {
    ...
    public void testImportUnencryptedPrivateKeysWithUnencryptedWallet() throws Exception {
        ...
        String testUnencryptedPrivateKeysFile = testDirectory + File.separator + PrivateKeysHandlerTest_SKIP.TEST1_PRIVATE_KEYS_FILE;
...
//        checkEveryExpectedKeyIsPresent(controller);
    ...
    public void testImportEncryptedPrivateKeysWithUnencryptedWallet() throws Exception {
        ...
        String testDirectory = currentPath + File.separator + Constants.TESTDATA_DIRECTORY + File.separator
                + PrivateKeysHandlerTest_SKIP.PRIVATE_KEYS_TESTDATA_DIRECTORY;
        String testEncryptedPrivateKeysFile = testDirectory + File.separator + PrivateKeysHandlerTest_SKIP.ENCRYPTED_TEST1_PRIVATE_KEYS_FILE;
        ...
        importPanel.setImportFilePassword(PrivateKeysHandlerTest_SKIP.ENCRYPTED_TEST1_PASSWORD);
...
//        checkEveryExpectedKeyIsPresent(controller);
    ...
    private void checkEveryExpectedKeyIsPresent(BitcoinController controller) {
        // Check every key on the expected list is now on the wallet.
        for (int i = 0; i < PrivateKeysHandlerTest_SKIP.EXPECTED_ADDRESSES_FOR_TEST1_WALLET.length; i++) {
            boolean foundIt = false;
            for (ECKey ecKey : controller.getModel().getActiveWallet().getKeychain()) {
                if (PrivateKeysHandlerTest_SKIP.EXPECTED_ADDRESSES_FOR_TEST1_WALLET[i].equals(ecKey.toAddress(controller.getModel().getNetworkParameters()).toString())) {
                    foundIt = true;
                    break;
                }
            }
            ...
            if (!foundIt) {
                fail("The address " + PrivateKeysHandlerTest_SKIP.EXPECTED_ADDRESSES_FOR_TEST1_WALLET[i] + " was not found on the wallet after an import.");
            }
        }
    }
}
